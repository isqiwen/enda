...
TEST(ItertoolsTest, Distance)
{
    ...
    EXPECT_EQ(enda::itertools::distance(it1, it2), std::distance(it1, it2));
    EXPECT_EQ(enda::itertools::distance(it3, it4), std::distance(it3, it4));
}

TEST(ItertoolsTest, MakeVectorFromRange)
{
...
TEST(ItertoolsTest, ChunkRange)
{
    // check the correctness of some chunked ranges
    EXPECT_EQ(chunk_range(0, 10, 1, 0), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(0l, 10l)));
    EXPECT_EQ(chunk_range(0, 10, 2, 0), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(0l, 5l)));
    EXPECT_EQ(chunk_range(0, 10, 2, 1), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(5l, 10l)));
    EXPECT_EQ(chunk_range(0, 10, 3, 0), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(0l, 4l)));
    EXPECT_EQ(chunk_range(0, 10, 3, 1), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(4l, 7l)));
    EXPECT_EQ(chunk_range(0, 10, 3, 2), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(7l, 10l)));
    EXPECT_EQ(chunk_range(0, 10, 4, 0), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(0l, 3l)));
    EXPECT_EQ(chunk_range(0, 10, 4, 1), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(3l, 6l)));
    EXPECT_EQ(chunk_range(0, 10, 4, 2), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(6l, 8l)));
    EXPECT_EQ(chunk_range(0, 10, 4, 3), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(8l, 10l)));
    EXPECT_EQ(chunk_range(0, 10, 11, 0), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(0l, 1l)));
    EXPECT_EQ(chunk_range(0, 10, 11, 10), (std::make_pair<std::ptrdiff_t, std::ptrdiff_t>(10l, 10l)));
}

TEST(ItertoolsTest, Enumerate)
{
...
TEST(ItertoolsTest, Zip)
{
    std::array<non_copyable_int, 6> arr {6, 5, 4, 3, 2, 1};
    std::vector<int>                vec1 {1, 2, 3, 4, 5, 6};

    // zip two ranges
    for (auto [x, y] : zip(arr, vec1))
    {
        EXPECT_TRUE(7 - y == x);
    }
    ...
    int                 count = 0;
    for ([[maybe_unused]] auto [x, y] : zip(arr, vec2))
    {
        ++count;
    }
...
TEST(ItertoolsTest, Product)
{
    ...
    for (auto [x, y] : product(vec2, vec3))
    {
        y *= x;
    }
    ...
    int                  count = 0;
    for ([[maybe_unused]] auto [u, v, w, x] : make_product(range_arr))
        ++count;
    EXPECT_EQ(count, 1 * 2 * 3 * 4);
}

TEST(ItertoolsTest, Slice)
{
...
TEST(ItertoolsTest, Stride)
{
    ...
    std::vector<int> vec2;
    int              empty_size = 0;
    for ([[maybe_unused]] auto x : stride(vec2, 2))
    {
        ++empty_size;
    }
...
TEST(ItertoolsTest, Range)
{
    ...
    for (int a = -L; a <= L; a++)
        for (int b = -L; b <= L; b++)
            for (int s = 1; s <= 3; s++)
            {
                ...
                for (auto i : range(a, b, (a <= b) ? s : -s))
                {
                    sum_with_range += static_cast<int>(i);
                }
                int sum_exact = 0;
                if (a <= b)
                    for (int i = a; i < b; i += s)
    ...
    long res = 0;
    for (auto [i, j, k] : product_range(5, 5, 5))
        res += i * j * k;
    EXPECT_EQ(res, 1000);
}

TEST(ItertoolsTest, CombinationOfRangeAdaptingFunctions)
{
    std::vector<int> vec1 {1, 2, 3, 4, 5, 6};
    ...
    for (auto [x1, x2] : zip(transform(vec1, enum_imitation), enumerate(vec1)))
    {
        EXPECT_TRUE(x1 == x2);
    }

    // enumerate a transformed range
    for (auto [i, x] : enumerate(transform(vec1, enum_imitation)))
    {
        std::cout << i << "  [" << std::get<0>(x) << ", " << std::get<1>(x) << "]\n";
    }
    ...
    int total = 0;
    for (auto sum : transform(product(vec1, vec1), add))
        total += sum;
    EXPECT_EQ(total, 252);

    // slice a zipped range
    for (auto [x1, x2] : slice(zip(vec1, vec1), 0, 4))
    {
        EXPECT_EQ(x1, x2);
    }
    ...
    total          = 0;
    for (auto [a, b] : product(transform(range(10), times_ten), range(10)))
    {
