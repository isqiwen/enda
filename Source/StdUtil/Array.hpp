...
namespace std
{
    ...
    template<typename T, size_t R>
    constexpr std::array<T, R> operator*(std::array<T, R> const& a, T s)
    {
        return s * a;
    }
...
namespace enda::stdutil
{
    ...
    {
        static_assert(std::is_constructible_v<T, U>, "Error in enda::stdutil::make_std_array: T must be constructible from U");
    ...
    {
        static_assert(R > 0, "Error in enda::stdutil::product: Only defined for R > 0");
        auto res = a[0];
        for (int i = 1; i < R; ++i)
            res *= a[i];
        return res;
