...
namespace enda
{
    ...
    FORCEINLINE void for_each_static(std::array<Int, R> const& shape, F&& f)
    { // NOLINT (we do not want to forward here)
    ...
    FORCEINLINE void for_each(std::array<Int, R> const& shape, F&& f)
    { // NOLINT (we do not want to forward here)
        auto idxs = enda::stdutil::make_initialized_array<R>(0l);
