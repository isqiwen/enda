...
namespace enda
{
    ...
    namespace detail
    {
        ...
        // Specialization of rect_str_from_base for enda::idx_map.
        template<int Rank, uint64_t StaticExtents, uint64_t StrideOrder, layout_prop_e LayoutProp>
     ...
     * linear/flat index.
     *
     * @details It extends the functionality of enda::idx_map by supporting string indices.
     *
    ...
    class rect_str : public idx_map<Rank, StaticExtents, StrideOrder, LayoutProp>
    {
        ...
        using ind_t = enda::array<enda::array<std::string, 1>, 1>;

        // Type of the enda::idx_map base class.
        ...
        // Get the shape of the enda::rect_str from given string indices.
    ...
    public:
        ...
        auto const& get_string_indices() const
        {
            if (not s_indices)
            {
                ...
                for (int i = 0; i < Rank; ++i)
                {
                    auto a = enda::array<std::string, 1>(this->lengths()[i]);
         ...
         * @details The string indices are not initialized and the underlying enda::idx_map is default constructed.
         */
        rect_str() = default;

        /**
         * @brief Construct an enda::rect_str from a given enda::idx_map.
         * @param idxm enda::idx_map object.
         */
        rect_str(base_t const& idxm) noexcept : base_t {idxm} {}

        /**
         * @brief Construct an enda::rect_str from a given enda::idx_map and string indices.
         *
         * @warning The shape of the string indices is not checked to be consistent with the shape of the enda::idx_map.
         *
         * @param idxm enda::idx_map object.
        ...
        /**
         * @brief Construct an enda::rect_str from another enda::rect_str with different layout properties.
         *
         * @tparam LP Layout properties of the other enda::rect_str.
         * @param rstr Other enda::rect_str object.
         */
        ...
        /**
         * @brief Construct an enda::rect_str from another enda::rect_str with different layout properties and static extents.
         *
         * @tparam SE Static extents of the other enda::rect_str.
         * @tparam LP Layout properties of the other enda::rect_str.
         * @param rstr Other enda::rect_str object.
         */
        ...
        /**
         * @brief Construct an enda::rect_str from a given shape and strides.
         *
        ...
        /**
         * @brief Construct an enda::rect_str from a given shape and with contiguous strides.
         * @param shape Shape of the map.
         */
        rect_str(std::array<long, Rank> const& shape) noexcept : base_t {shape} {}

        /**
         * @brief Construct an enda::rect_str from given string indices and with contiguous strides.
         * @param str_indices String indices.
         */
        rect_str(enda::array<enda::array<std::string, 1>, 1> str_indices) noexcept(false) :
            base_t {make_shape_from_string_indices(str_indices)}, s_indices {std::make_shared<ind_t>(std::move(str_indices))}
        {}

        /**
         * @brief Construct an enda::rect_str from an array with its dynamic extents.
    ...
    private:
        ...
        {
            // calls the underlying enda::idx_map::operator() after converting all string arguments to long indices
    ...
    private:
        // Helper function to get a new enda::rect_str by taking a slice of the current one.
        template<typename... Args, auto... Is>
        FORCEINLINE decltype(auto) slice_impl(std::index_sequence<Is...>, Args const&... args) const
        {
            // convert string arguments to long indices and slice the underlying enda::idx_map
            auto const [offset, idxm2] = base_t::slice(peel_string(Is, args)...);

            // type of sliced enda::rect_str
            using new_rect_str_t = typename detail::rect_str_from_base<std::decay_t<decltype(idxm2)>>::type;

            // if the string indices have not been initialized, simply return a new enda::rect_str with the sliced enda::idx_map
            if (not s_indices)
            ...
            auto        add_string_indices = [p = 0, &current_ind, &ind2](int n, auto const& y) mutable -> void {
                ...
                if constexpr (not argument_is_allowed_for_call<U>)
                {
                    ind2[p++] = current_ind[n](y);
                }
            };
    ...
    public:
        /**
         * @brief Get a new enda::rect_str by taking a slice of the current one.
         *
         * @warning enda::ellipsis that cover more than 1 dimension will not work properly. Use `enda::range::all_t` instead.
         *
         * @tparam Args Types of the arguments.
         * @param args Multi-dimensional index consisting of strings, `long`, `enda::range`, `enda::range::all_t` or
         * enda::ellipsis objects.
         * @return A std::pair containing the offset in memory, i.e. the flat index of the first element of the slice and
         * the new enda::rect_str.
         */
        template<typename... Args>
        ...
        /**
         * @brief Equal-to operator for two enda::rect_str objects.
         *
         * @param rhs Right hand side enda::rect_str operand.
         * @return True if the underlying enda::idx_map and the string indices are equal, false otherwise.
         */
        bool operator==(rect_str const& rhs) const { return base_t::operator==(rhs) and (!s_indices or !rhs.s_indices or (*s_indices == *(rhs.s_indices))); }

        /**
         * @brief Not-equal-to operator for two enda::rect_str objects.
         *
         * @param rhs Right hand side enda::rect_str operand.
        ...
        /**
         * @brief Create a new enda::rect_str by permuting the indices/dimensions with a given permutation.
         *
         * @details Let `A` be the current and ``A'`` the new, permuted map. `P` is the given permutation. We define the
         * permuted enda::rect_str ``A'`` to be the one with the following properties:
         ...
         * @return New enda::rect_str with permuted indices.
         */
        template<uint64_t Permutation>
        auto transpose() const
        {
            // transpose the underlying enda::idx_map
            auto idxm2 = base_t::template transpose<Permutation>();

            // type of transposed enda::rect_str
            using new_rect_str_t = typename detail::rect_str_from_base<std::decay_t<decltype(idxm2)>>::type;

            // if the string indices have not been initialized, simply return the transposed enda::rect_str
            if (not s_indices)
    ...
    namespace detail
    {
        ...
        // Get the correct layout policy given a general enda::rect_str.
        template<int Rank, uint64_t StaticExtents, uint64_t StrideOrder, layout_prop_e LayoutProp>
        ...
        // Get the correct layout policy given a strided enda::rect_str with C-order.
        template<int Rank>
