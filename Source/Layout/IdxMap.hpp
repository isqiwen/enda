...
#include "Layout/SliceStatic.hpp"
#include "Macros.hpp"
#include "Traits.hpp"
...
namespace enda
{
     ...
     */
    template<int Rank>
    constexpr uint64_t Fortran_stride_order = enda::encode(enda::permutations::reverse_identity<Rank>());
     ...
     */
    template<int Rank>
    constexpr uint64_t C_stride_order = enda::encode(enda::permutations::identity<Rank>());
     ...
     * `std::array<int, Rank>` using the enda::decode function. The encoding limits the number of dimensions, i.e. the
     * rank, to 16.
     *
     ...
     * - `StaticExtents = enda::encode(std::array<int, Rank>{0, 10, 0, 5})` corresponds to a 4D array with dynamic extents
     ...
     * - C-order: `StrideOrder = enda::encode(std::array<int, Rank>{0, 1, ..., Rank - 2, Rank - 1})`.
     * - Fortran-order: `StrideOrder = enda::encode(std::array<int, Rank>{Rank - 1, Rank - 2, ..., 1, 0})`.
     *
     * `LayoutProp` is an enum flag that can be used to make further compile-time guarantees about the layout of the data
     * in memory (see enda::layout_prop_e).
     *
     * @tparam Rank Number of dimensions.
    ...
    class idx_map
    {
        static_assert(Rank < 16, "Error in enda::idx_map: Rank must be < 16");
        static_assert((StrideOrder != 0) or (Rank == 1), "Error in enda::idx_map: StrideOrder can only be zero for 1D arrays");
    ...
    private:
            ...
            if constexpr (n_dynamic_extents != Rank)
            {
                ...
                for (int u = 0; u < Rank; ++u)
                    if (static_extents[u] != 0)
                        EXPECTS(static_extents[u] == len[u]);
    #endif
            }
#endif
        }
        ...
        static std::array<long, Rank> merge_static_and_dynamic_extents(std::array<long, n_dynamic_extents> const& dynamic_extents)
        {
            ...
            for (int u = 0, v = 0; u < Rank; ++u)
                extents[u] = (static_extents[u] == 0 ? dynamic_extents[v++] : static_extents[u]);
            return extents;
        }
    ...
    public:
         ...
         *
         * @tparam LP Layout properties of the other enda::idx_map.
         * @param idxm Other enda::idx_map object.
         */
        ...
        idx_map(idx_map<Rank, StaticExtents, StrideOrder, LP> const& idxm) noexcept : len(idxm.lengths()), str(idxm.strides())
        {
            ...
            if constexpr (not layout_property_compatible(LP, layout_prop))
            {
                ...
                {
                    EXPECTS_WITH_MESSAGE(idxm.is_contiguous(), "Error in enda::idx_map: Constructing a contiguous from a non-contiguous layout");
                }
                if constexpr (has_strided_1d(layout_prop))
                {
                    EXPECTS_WITH_MESSAGE(idxm.is_strided_1d(), "Error in enda::idx_map: Constructing a strided_1d from a non-strided_1d layout");
                }
            }
        }
         ...
         *
         * @tparam SE Static extents of the other enda::idx_map.
         * @tparam LP Layout properties of the other enda::idx_map.
         * @param idxm Other enda::idx_map object.
         */
        ...
        idx_map(std::array<long, Rank> const& shape, // NOLINT (only throws if check_stride_order is true)
                std::array<long, Rank> const& strides) noexcept(!check_stride_order) : len(shape), str(strides)
        {
            ...
            if constexpr (check_stride_order)
            {
                if (not is_stride_order_valid())
                    throw std::runtime_error("Error in enda::idx_map: Incompatible strides, shape and stride order");
            }
        }
         ...
         *
         * @tparam SE Static extents of the other enda::idx_map.
         * @tparam SO Stride order of the other enda::idx_map.
         * @tparam LP Layout properties of the other enda::idx_map.
         */
        template<uint64_t SE, uint64_t SO, layout_prop_e LP>
        idx_map(idx_map<Rank, SE, SO, LP> const&) requires(stride_order_encoded != SO)
        {
            static_assert((stride_order_encoded == SO), "Error in enda::idx_map: Incompatible stride orders");
        }
    ...
    private:
        // Get the contribution to the linear index in case of an enda::ellipsis argument.
        template<bool skip_stride, auto I>
        [[nodiscard]] FORCEINLINE long myget(ellipsis) const noexcept
        {
            // enda::ellipsis are skipped and do not contribute to the linear index
            return 0;
        }
    ...
    public:
         ...
         *
         * @details All arguments are either convertible to type `long` or are of type enda::ellipsis. The number of
         * non-ellipsis arguments must be equal to the rank of the map and there must be at most one enda::ellipsis. If an
         * enda::ellipsis is present, it is skipped and does not influence the result.
         *
        ...
        /**
         * @brief Get a new enda::idx_map by taking a slice of the current one.
         *
         * @details See enda::slice_static::slice_idx_map for more information.
         *
         * @tparam Args Types of the arguments.
         * @param args Multi-dimensional index consisting of `long`, `enda::range`, `enda::range::all_t` or enda::ellipsis
         * objects.
         * @return A std::pair containing the offset in memory, i.e. the flat index of the first element of the slice and
         * the new enda::idx_map.
         */
        template<typename... Args>
        ...
        /**
         * @brief Equal-to operator for two enda::idx_map objects.
         *
         * @tparam R Rank of the other enda::idx_map.
         * @tparam SE Static extents of the other enda::idx_map.
         * @tparam SO Stride order of the other enda::idx_map.
         * @tparam LP Layout properties of the other enda::idx_map.
         * @param rhs Right hand side enda::idx_map operand.
         ...
         * the permuted enda::idx_map ``A'`` to be the one with the following properties:
         ...
         * @return New enda::idx_map with permuted indices.
         */
