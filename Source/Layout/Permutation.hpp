...
namespace enda
{
    ...
    constexpr std::array<int, N> decode(uint64_t binary_representation)
    {
        ...
        for (int i = 0; i < N; ++i)
            result[i] = (binary_representation >> (4 * i)) & 0b1111ull;
        return result;
    }
...
namespace enda::permutations
{
    ...
    constexpr bool is_valid(std::array<Int, N> const& p)
    {
        ...
        for (auto idx : p)
        {
            if (idx < 0 or idx > N - 1 or idx_counts[idx] > 0)
                return false;
            idx_counts[idx] = 1;
        }
        return true;
     ...
     *
     * @details See also enda::permutations::apply and enda::permutations::inverse.
     *
