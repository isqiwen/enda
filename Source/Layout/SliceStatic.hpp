...
namespace enda::slice_static
{
    ...
    namespace detail
    {
        ...
        // Helper function to get the position of an enda::ellipsis object in a given parameter pack.
        ...
        // Get the position of an enda::ellipsis object in a given parameter pack (returns 128 if there is no enda::ellipsis).
        ...
        constexpr layout_prop_e slice_layout_prop(int                        P,
                                                  bool                       has_only_rangeall_and_long,
                                                  std::array<bool, Q> const& args_is_rangeall,
                                                  std::array<int, N> const&  orig_stride_order,
                                                  ...
                                                  int                        e_len)
        {
            ...
            // count the number of enda::range::all_t blocks in the argument list, e.g. long, range::all, range::all, long,
            // range::all -> 2 blocks
            ...
            for (int i = 0; i < N; ++i)
            {
                ...
                if (arg_is_rangeall and (not previous_arg_is_rangeall))
                    ++n_rangeall_blocks;
            ...
            if (has_contiguous(orig_layout_prop) and rangeall_are_grouped_in_memory and last_is_rangeall)
                return layout_prop_e::contiguous;
            if (has_strided_1d(orig_layout_prop) and rangeall_are_grouped_in_memory)
                return layout_prop_e::strided_1d;
            if (has_smallest_stride_is_one(orig_layout_prop) and last_is_rangeall)
                return layout_prop_e::smallest_stride_is_one;
        ...
        // Get the contribution to the flat index of the first element of the slice from a single dimension if the argument
        // is a range::all_t or covered by an enda::ellipsis.
...
#ifdef ENDA_ENFORCE_BOUNDCHECK
            enda::assert_in_bounds(idxm.rank(), idxm.lengths().data(), args...);
#endif
            // compile time check
    ...
    /**
     * @brief Determine the resulting enda::idx_map when taking a slice of a given enda::idx_map.
     *
     * @details Let `n_args` be the number of given `long`, `enda::range`, `enda::range::all_t` or enda::ellipsis arguments.
     *
     * The rank ``R'`` of the resulting slice is determined by the rank `R` of the original enda::idx_map and the number
     * `n_long` of `long` arguments, i.e. ``R' = R - n_long``.
     *
     * The number of allowed enda::ellipsis objects is restricted to at most one. If an enda::ellipsis object is present and
     * `n_args <= R`, the ellipsis is expanded in terms of `enda::range::all_t` objects to cover the remaining
     * `R - n_args + 1` dimensions. Otherwise, the ellipsis is ignored.
     *
     * After ellipsis expansion, the only arguments contributing to the slice are `enda::range` and `enda::range::all_t`
     * objects. Together with the original enda::idx_map, they determine the resulting enda::idx_map.
     *
     * @tparam R Rank of the original enda::idx_map.
     * @tparam SE Static extents of the original enda::idx_map.
     * @tparam SO Stride order of the original enda::idx_map.
     * @tparam LP Layout properties of the original enda::idx_map.
     * @tparam Args Given argument types.
     * @param idxm Original enda::idx_map.
     * @param args Arguments consisting of `long`, `enda::range`, `enda::range::all_t` or enda::ellipsis objects.
     * @return Resulting enda::idx_map of the slice.
     */
    ...
    FORCEINLINE decltype(auto) slice_idx_map(idx_map<R, SE, SO, LP> const& idxm, Args const&... args)
    {
        ...
        // compile time checks
        static_assert(n_args_ellipsis <= 1, "Error in enda::slice_static::slice_idx_map: At most one ellipsis argument is allowed");
        static_assert((sizeof...(Args) <= R + 1), "Error in enda::slice_static::slice_idx_map: Incorrect number of arguments");
        static_assert((n_args_ellipsis == 1) or (sizeof...(Args) == R), "Error in enda::slice_static::slice_idx_map: Incorrect number of arguments");
