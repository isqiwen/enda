/**
 * @file GroupIndices.hpp
 *
 * @brief Provides functions used in enda::group_indices_view.
 */
...
namespace enda
{
    namespace detail
    {
        ...
        constexpr bool is_partition_of_indices(std::array<int, Rs> const&... grps)
        {
            ...
            static_assert(Rstot == R, "Error in enda::detail::is_partition_of_indices: Number of indices has to be equal to the rank");
            ...
            auto l     = [&](auto&& grp) mutable -> void {
                ...
                {
                    if (grp[u] < 0)
                        throw std::runtime_error("Error in enda::detail::is_partition_of_indices: Negative indices are not allowed");
                    if (grp[u] >= R)
                        throw std::runtime_error("Error in enda::detail::is_partition_of_indices: Indices larger than the rank are not allowed");
                    count[grp[u]]++;
                }
            };
            (l(grps), ...);
        ...
        // enda::idx_map.
        template<size_t R, size_t... Rs>
        constexpr std::array<int, sizeof...(Rs)> stride_order_of_grouped_idx_map(std::array<int, R> const& stride_order, std::array<int, Rs> const&... grps)
        {
            ...
            for (int u = 0; u < Ngrps; ++u)
            {
                ...
                {
                    if (min_mem_positions[i] < min_mem_positions[u])
                        ++mem_pos_out[u];
                }
            }
     ...
     *
     * @details It is used to create an enda::group_indices_view: `group_indices_view(A, idx_group<i,j,...>,
     ...
     *
     * @tparam Rank Rank of the original enda::idx_map.
     * @tparam StaticExtents Static extents of the original enda::idx_map.
     * @tparam StrideOrder Stride order of the original enda::idx_map.
     * @tparam LayoutProp Layout property of the original enda::idx_map.
     * @tparam IdxGrps Groups of indices.
     *
     * @param idxm Original enda::idx_map.
     * @return New enda::idx_map with the grouped indices.
     */
    ...
    {
        // compile-time checks
        static_assert(LayoutProp == layout_prop_e::contiguous, "Error in enda::group_indices_layout: Only contiguous layouts are supported");
        static_assert(detail::is_partition_of_indices<Rank>(IdxGrps::as_std_array...),
                      "Error in enda::group_indices_layout: Given index groups are not a valid partition");
