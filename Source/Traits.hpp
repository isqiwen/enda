...
namespace enda
{
     ...
     *
     * @code{.cpp}
     * static_assert(enda::is_instantiation_of<std::complex, std::complex<double>>::value);
     * static_assert(not enda::is_instantiation_of<std::complex, double>::value);
     * @endcode
     *
    ...
    /**
     * @brief Constexpr variable that is true if type `S` is a scalar type (see enda::is_scalar_v) or if a
    ...
    /**
     * @brief Constexpr variable used to check requirements when initializing an enda::basic_array or enda::basic_array_view
    ...
    /// Alias for enda::is_double_or_complex_v.
    template<typename T>
     ...
     *
     * @param from Source enda::layout_prop_e.
     * @param to Target enda::layout_prop_e.
     * @return True if the source layout property is greater or equal to the target layout property,
     * otherwise false (see enda::layout_prop_e).
     */
     ...
     *
     * @param lhs Left hand side enda::layout_prop_e operand.
     * @param rhs Right hand side enda::layout_prop_e operand.
     ...
     *
     * @param lp enda::layout_prop_e to check.
     ...
     *
     * @param lhs Left hand side enda::layout_info_t operand.
     * @param rhs Right hand side enda::layout_info_t operand.
     * @return A new enda::layout_info_t with the same stride order as the two arguments and their compatible
     * enda::layout_prop_e. If the stride orders are different, the stride order is set to -1 (undefined, no
