...
namespace std
{
    ...
    /**
     * @brief std::swap is deleted for enda::basic_array_view.
     * @warning The std::swap is WRONG for an enda::basic_array_view because of its copy/move semantics. Use enda::swap
    ...
    void swap(enda::basic_array_view<V1, R1, LP1, A1, AP1, OP1>& a, enda::basic_array_view<V2, R2, LP2, A2, AP2, OP2>& b) = delete;
...
namespace enda
{
     ...
     *
     * @details Together with enda::basic_array, this class forms the backbone of the nda library. It is templated with
    ...
    {
        // Compile-time checks.
        static_assert((Algebra != 'N'), "Internal error in enda::basic_array_view: Algebra 'N' not supported");
        static_assert((Algebra != 'M') or (Rank == 2), "Internal error in enda::basic_array_view: Algebra 'M' requires a rank 2 view");
    ...
    private:
        ...
        // Memory layout of the view, i.e. the enda::idx_map.
        layout_t lay;

        // Memory handle of the view.
        storage_t sto;

        // Declare any enda::basic_array as a friend.
        template<typename T, int R, typename L, char A, typename CP>
        friend class basic_array;

        // Declare any other enda::basic_array_view as a friend.
    ...
    public:
         ...
         * enda::MemoryArray object.
         *
         * @tparam A enda::MemoryArray type of the same rank as the view.
         * @param a enda::MemoryArray object.
         */
        template<MemoryArrayOfRank<Rank> A>
        requires((get_layout_info<A>.stride_order == layout_t::stride_order_encoded) and (std::is_same_v<std::remove_const_t<ValueType>, get_value_t<A>>) and
                 (std::is_const_v<ValueType> or
                  !std::is_const_v<typename std::decay_t<A>::value_type>)) explicit(requires_runtime_check<typename std::decay_t<A>::layout_policy_t>)
         ...
         */
        template<size_t N>
        requires(Rank == 1) explicit basic_array_view(std::array<ValueType, N>& a) noexcept : basic_array_view {{long(N)}, a.data()}
        {}
         ...
         */
        template<size_t N>
        requires(Rank == 1 and std::is_const_v<ValueType>) explicit basic_array_view(std::array<std::remove_const_t<ValueType>, N> const& a) noexcept :
        ...
        requires(Rank == 1 and not MemoryArray<R> and
                 (std::is_same_v<std::ranges::range_value_t<R>, ValueType> or
                  std::is_same_v<const std::ranges::range_value_t<R>, ValueType>)) explicit basic_array_view(R& rg) noexcept :
            basic_array_view {{long(std::ranges::size(rg))}, std::to_address(std::begin(rg))}
        {}
         ...
         * `ENDA_ENFORCE_BOUNDCHECK` is defined, bounds checking is enabled.
         *
        ...
        /**
         * @brief Assignment operator makes a deep copy of the contents of an enda::ArrayOfRank object.
         *
         * @details The dimension of the right hand side must be large enough or the behaviour is undefined. If
         * `ENDA_ENFORCE_BOUNDCHECK` is defined, bounds checking is enabled.
         *
         * @tparam RHS enda::ArrayOfRank type with the same rank as the view.
         * @param rhs Right hand side of the assignment operation.
         */
        template<ArrayOfRank<Rank> RHS>
        basic_array_view& operator=(RHS const& rhs) noexcept
        {
            static_assert(!is_const, "Cannot assign to an enda::basic_array_view with const value_type");
            assign_from_ndarray(rhs);
            return *this;
        }
        ...
        /**
         * @brief Assignment operator uses an enda::ArrayInitializer to assign to the view.
         *
...
#include "ImplBasicArrayViewCommon.hpp"
    ...
    basic_array_view(A&& a) -> basic_array_view<
        std::conditional_t<std::is_const_v<std::remove_reference_t<A>>, const typename std::decay_t<A>::value_type, typename std::decay_t<A>::value_type>,
        get_rank<A>,
        typename std::decay_t<A>::layout_policy_t,
        get_algebra<A>,
        default_accessor,
    ...
    basic_array_view(std::array<V, R>& a)
        -> basic_array_view<V,
                            1,
                            enda::basic_layout<enda::static_extents(R), enda::C_stride_order<1>, enda::layout_prop_e::contiguous>,
                            'V',
                            default_accessor,
                            borrowed<>>;
    ...
    template<std::ranges::contiguous_range R>
    basic_array_view(R& r) -> basic_array_view<std::conditional_t<std::is_const_v<R>, const typename R::value_type, typename R::value_type>,
                                               1,
                                               C_layout,
                                               'V',
                                               default_accessor,
