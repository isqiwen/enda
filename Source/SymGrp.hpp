...
namespace enda
{
    ...
    /**
     * @brief Check if a multi-dimensional index is valid, i.e. not out of bounds, w.r.t. to a given enda::Array object.
     *
    ...
    bool is_valid(A const& a, std::array<long, static_cast<std::size_t>(get_rank<A>)> const& idx)
    {
        ...
        {
            if (not(0 <= idx[i] and idx[i] < a.shape()[i]))
            {
                return false;
            }
        }
     ...
     * rank as a given array type and returns a tuple with a new multi-dimensional index and an enda::operation.
     *
    ...
    concept NdaSymmetry = Array<A> and requires(F f, Idx const& idx)
    {
        {
            f(idx)
        } -> std::same_as<std::tuple<Idx, operation>>;
    };
    ...
    concept NdaInitFunc = Array<A> and requires(F f, Idx const& idx)
    {
        {
            f(idx)
        } -> std::same_as<get_value_t<A>>;
    };

    /**
     * @brief Class representing a symmetry group.
     *
     * @details A symmetry group detects and stores all different symmetry classes associated with a given enda::Array
     * object.
     *
     ...
     * i.e. an enda::operation. The symmetry classes form a partition of all the elements of the array.
     *
     * @tparam F enda::NdaSymmetry type.
     * @tparam A enda::Array type.
     */
    ...
    public:
        ...
        /**
         * @brief Initialize an enda::Array using an enda::NdaInitFunc.
         *
         * @details The enda::NdaInitFunc object is evaluated only once per symmetry class. The result is then assigned to
         * all elements in the symmetry class after applying the enda::operation.
         *
         * @tparam H Callable type of enda::NdaInitFunc.
         * @param a enda::Array object to be initialized.
         * @param init_func Callable that is used to initialize the array.
         * @param parallel Parallelize using openmp and mpi.
         */
        template<typename H>
        requires(NdaInitFunc<H, A>) void init(A& a, H const& init_func, bool parallel = false) const
        {
            if (parallel)
            {
...
#pragma omp parallel
                for (auto const& sym_class : itertools::omp_chunk(mpi::chunk(sym_classes)))
                {
                    ...
                    {
                        std::apply(a, a.indexmap().to_idx(lin_idx)) = op(ref_val);
                    }
                }
         ...
         *
         * @param a enda::Array object to be symmetrized.
        ...
        std::pair<double, arr_idx_t> symmetrize(A& a) const
        {
            ...
            for (auto const& sym_class : sym_classes)
            {
                ...
                for (auto const& [lin_idx, op] : sym_class)
                {
                    ref_val += op(std::apply(a, a.indexmap().to_idx(lin_idx)));
                }
        ...
        /**
         * @brief Reduce an enda::Array to its representative data using symmetries.
         *
         * @param a enda::Array object.
        ...
        [[nodiscard]] std::vector<get_value_t<A>> get_representative_data(A const& a) const
        {
            ...
            for (long i : range(len))
                vec[i] = std::apply(a, a.indexmap().to_idx(sym_classes[i][0].first));
            return vec;
        }
        ...
        void init_from_representative_data(A& a, V const& vec) const
        {
            ...
            {
                for (auto const& [lin_idx, op] : sym_classes[i])
                {
                    std::apply(a, a.indexmap().to_idx(lin_idx)) = op(vec[i]);
                }
            }
        }
         ...
         *
         * @details It uses enda::for_each to loop over all possible multi-dimensional indices of the given array and applies
         * the symmetries to each index to generate the different symmetry classes.
         *
         * @param a enda::Array object.
         * @param sym_list List of symmetries containing enda::NdaSymmetry objects.
    ...
    private:
        ...
        long long iterate(std::array<long, static_cast<std::size_t>(get_rank<A>)> const& idx,
                          operation const&                                               op,
                          array<bool, ndims>&                                            checked,
