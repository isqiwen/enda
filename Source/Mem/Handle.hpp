...
namespace enda::mem
{
    ...
    // Instance of enda::mem::do_not_initialize_t.
     ...
     *
     * @details By default it uses enda::mem::mallocator to allocate/deallocate memory.
     *
     * @tparam T Value type of the data.
     * @tparam A enda::mem::Allocator type.
     */
    template<typename T, Allocator A = mallocator<>>
    struct handle_heap
    {
        static_assert(std::is_nothrow_destructible_v<T>, "enda::mem::handle_heap requires the value_type to have a non-throwing destructor");
    ...
    #define HANDLE_HEAP_MALLOC_ZERO(size) ENDA_MALLOC_ZERO_STATS(allocator, size);
        ...
        static void destruct(blk_T_t b) noexcept
        {
            ...
            if constexpr (A::address_space == Host and !(std::is_trivial_v<T> or enda::is_complex_v<T>))
            {
    ...
    public:
        ...
        {
            if (not sptr)
                sptr.reset(new blk_T_t {_data, _size}, deleter);
            return sptr;
        }
         ...
         *
         * @tparam H enda::mem::OwningHandle type.
         * @param h Source handle.
         */
        ...
        explicit handle_heap(H const& h) : handle_heap(h.size(), do_not_initialize)
        {
            ...
            else
            {
                static_assert(address_space == H::address_space,
                              "Constructing an enda::mem::handle_heap from a handle of a different address space requires a trivially copyable value_type");
                for (size_t i = 0; i < _size; ++i)
                    new (_data + i) T(h[i]);
            }
        }
         ...
         * - If `T` is std::complex and enda::mem::init_dcmplx is true, the data is initialized to zero.
     ...
     *
     * @note This handle only works with the `Host` enda::mem::AddressSpace.
     *
     * @tparam T Value type of the data.
    ...
    struct handle_stack
    {
        static_assert(std::is_copy_constructible_v<T>, "enda::mem::handle_stack requires the value_type to be copy constructible");
        static_assert(std::is_nothrow_destructible_v<T>, "enda::mem::handle_stack requires the value_type to have a non-throwing destructor");
     ...
     * stack, otherwise it is stored on the heap (enda::mem::mallocator is used to allocate/deallocate memory on the heap).
     *
    ...
    struct handle_sso
    {
        static_assert(Size > 0, "Size == 0 makes no sense in enda::mem::handle_sso");
        static_assert(std::is_copy_constructible_v<T>, "enda::mem::handle_sso requires the value_type to be copy constructible");
        static_assert(std::is_nothrow_destructible_v<T>, "enda::mem::handle_sso requires the value_type to have a non-throwing destructor");
    ...
    private:
        ...
        void clean() noexcept
        {
            ...
            if (on_heap())
                mallocator<>::deallocate({(char*)_data, _size * sizeof(T)});
            _data = nullptr;
            _size = 0;
        }
    ...
    public:
        ...
        explicit handle_sso(H const& h) : handle_sso(h.size(), do_not_initialize)
        {
            ...
            {
                static_assert(address_space == H::address_space,
                              "Constructing an enda::mem::handle_sso from a handle of a different address space requires a trivially copyable value_type");
                for (size_t i = 0; i < _size; ++i)
                    new (_data + i) T(h[i]);
            }
        }
        ...
        {
            static_assert(std::is_scalar_v<T> or is_complex_v<T>, "enda::mem::handle_sso can only be initialized to zero for scalar and complex types");
            if (size == 0)
                return;
            _size = size;
            if (not on_heap())
    ...
    struct handle_shared
    {
        static_assert(std::is_nothrow_destructible_v<T>, "enda::mem::handle_shared requires the value_type to have a non-throwing destructor");
    ...
    public:
        ...
        /**
         * @brief Construct a shared handle from an enda::mem::handle_heap.
         *
         * @tparam A enda::mem::Allocator type of the source handle.
         * @param h Source handle.
         */
        template<Allocator A>
        handle_shared(handle_heap<T, A> const& h) noexcept requires(A::address_space == address_space) : _data(h.data()), _size(h.size())
        {
            if (not h.is_null())
                sptr = h.get_sptr();
        }
    ...
    public:
         ...
         *
         * @tparam H enda::mem::Handle type.
         * @param h Other handle.
         * @param offset Pointer offset from the start of the data (in number of elements).
         */
        template<Handle H>
        requires(address_space == H::address_space and (std::is_const_v<value_type> or !std::is_const_v<typename H::value_type>) and
                 std::is_same_v<const value_type, const typename H::value_type>) handle_borrowed(H const& h, long offset = 0) noexcept :
            _data(h.data() + offset)
        {
            if constexpr (std::is_same_v<H, handle_heap<T0>>)
                _parent = &h;
        }
